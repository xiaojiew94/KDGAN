...
class TCH():
  def __init__(self, flags, dataset, is_training=True):
    ...
    with tf.variable_scope(tch_scope) as scope:
      ...
      # co kd train
      cokd_losses = self.get_cokd_losses(flags)
      self.cokd_loss = tf.add_n(cokd_losses, name='%s_cokd_losses' % tch_scope)
      cokd_optimizer = utils.get_opt(flags, self.learning_rate)
      self.cokd_update = cokd_optimizer.minimize(self.cokd_loss, global_step=self.global_step)
  ...
  def get_cokd_losses(self, flags):
    ## from get_kd_losses distn
    hard_loss = self.get_hard_loss()
    hard_loss *= (1.0 - flags.kd_soft_pct) / flags.batch_size
    std_logits = self.logits * (1.0 / flags.temperature)
    tch_logits = self.soft_logit_ph * (1.0 / flags.temperature)
    ...
    soft_loss *= flags.intellstd_weight

    cokd_losses = [hard_loss, soft_loss]
    return cokd_losses
