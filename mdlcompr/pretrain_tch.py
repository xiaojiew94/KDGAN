...
if flags.log_model_size:
  with open(flags.size_file, 'r') as fin:
    for line in fin.readlines():
      pass
  gen_params = int(line.split()[2])
  gen_compr = 100.0 * gen_params / tot_params
  with open(flags.size_file, 'a') as fout:
    m_params = tot_params / pow(1024, 2.0)
    fout.write('tch %d %d %.2fM ratio=%.0f\n' % (
        flags.hidden_size, tot_params, m_params, gen_compr))
  exit()
