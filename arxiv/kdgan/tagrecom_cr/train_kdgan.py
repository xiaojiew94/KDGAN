...
def main(_):
  ...
  with tf.train.MonitoredTrainingSession() as sess:
    ...
    for epoch in range(flags.num_epoch):
      ...
      num_batch_t = math.ceil(flags.num_tch_epoch * tn_data_size / flags.batch_size)
      ...
      for _ in range(num_batch_g):
        ...
        if prec < best_prec:
          continue
        # gen_hit = utils.evaluate_image(flags, sess, vd_gen, bt_list_v)
        # tch_hit = utils.evaluate_text(flags, sess, vd_tch, bt_list_v)

        # tot_time = time.time() - start
        # print('#%03d curgen=%.4f curtch=%.4f %.0fs' % (epoch, gen_hit, tch_hit, tot_time))
        # figure_data.append((epoch, gen_hit, tch_hit))
        # if gen_hit <= best_prec:
        #   continue
        # best_prec = gen_hit
  tot_time = time.time() - start
