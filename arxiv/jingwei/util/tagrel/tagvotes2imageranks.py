...
from basic.common import ROOT_PATH,writeRankingResults,checkToSkip
from basic.annotationtable import readConcepts

def readLabeledImageSet(collection, concept, tpp, rootpath):
    imsetfile = os.path.join(rootpath, collection, 'tagged,%s'% tpp, '%s.txt' % concept)
    return map(str.strip, open(imsetfile).readlines())
